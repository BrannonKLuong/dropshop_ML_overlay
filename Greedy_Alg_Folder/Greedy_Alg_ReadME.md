...
In this case, the formula can be the frames * d(log(d)) + % of failures * frames * d^2 == frames * d^2. Since the values may vary on a case-by-case scenario it's best to plot the graphs on a demos chart. Consider the following graphs. By tweaking the coefficient 0.2 on the case-by-case scenario it can be graphed what the computational difference would look like. This graph's x-axis is the number of droplets and the y-axis is the number of searches. 
