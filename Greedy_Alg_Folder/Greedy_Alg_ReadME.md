...
Therefore the core of the old implementation was to check every detection with every droplet O(n * d) since d == n, n * n, or O(n^2). In the Greedy Algorithm, the implementation constantly disregards half of the array so the search is now O(d * log2(n) and since d == n, O(n*log2n) or just O(nlogn). For an average of 4 droplets a frame that would be mean the original algorithm would've run 4^4 * 5750 = 92000 searches. Compared to the new algorithm which does binary search 98% of the time 0.98 * 5750(4log(4)) + 0.02(5750*4^2) or about 15410 searches.
